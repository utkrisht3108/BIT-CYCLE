...
async function show_data(){
    ...
    console.log(user);
    var temp=gettemplate(user.user.name,user.user.email,user.user.hostel,user.user.room);
    var temp_div = document.createElement('div');
    temp_div.innerHTML = temp;
    myaccount.innerHTML="";
    myaccount.appendChild(temp_div);
    if(transaction_arr.length==0){
      myaccount.innerHTML+="<div class='transaction-display'><h4>No transactions</h4></div>";
    }
    else{
